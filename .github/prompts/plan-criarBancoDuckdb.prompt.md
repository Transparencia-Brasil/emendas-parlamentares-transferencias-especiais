...
**TL;DR** — Criar a task `tasks/db/` com um script R que lê os 13 CSVs de [tasks/transferegov/outputs/](tasks/transferegov/outputs/), converte tipos (valores → `DOUBLE`, datas → `DATE`/`TIMESTAMP`), e grava num banco DuckDB único em `tasks/db/outputs/transferegov.duckdb` com chaves primárias e estrangeiras. O script expõe funções reutilizáveis para criar, popular e conectar ao banco.
...
2. **Implementar `utils.R` com 5 funções**:
   ...
   | `ler_csvs_transferegov()` | Reutiliza a lógica de `read_transferegov_csvs()` já existente em [00-load-transferegov.R](tasks/analise-exploratoria/src/R/00-load-transferegov.R#L25-L35): lista CSVs, lê como character, nomeia em snake_case sem prefixo numérico |
...
3. **Definir o schema SQL com constraints** — A função `criar_tabelas()` emitirá DDL para as 13 tabelas. Mapeamento de PK/FK baseado no fluxo de [coleta.R](tasks/transferegov/src/R/coleta.R) e na documentação do `copilot-instructions.md`:
   ...
   Os tipos SQL serão: `VARCHAR` (padrão), `DOUBLE` (colunas de valor/quantidade), `DATE` (colunas de data), `TIMESTAMP` (colunas data_hora).
...
5. **Ordem de inserção** (para respeitar FK):
   1. `programa`
   2. `plano_acao`
   3. `empenho`, `relatorio_gestao_novo`, `executor`, `plano_trabalho`
   4. `documento_habil`, `meta`, `finalidade`, `plano_trabalho_analise`, `orgao_analise_pendente`
   5. `ordem_pagamento`
   6. `historico_pagamento`
